...
# Page config
st.set_page_config(
    page_title="Cervical Cancer 5-year Prediction System",
    page_icon="🏥",
    layout="wide"
)

# Title
st.title("🏥 Cervical Cancer 5-year Recurrence Prediction System")
st.markdown("""
Cervical Cancer 5-year Recurrence Prediction System Based on XGBoost, with SHAP interpretability analysis.
...
feature_descriptions = {
    ...
    'FIGO_Stage': 'FIGO Stage - Higher stage indicates higher recurrence risk',
    'TumorSize_cm': 'Tumor Size (cm) - Larger tumors increase recurrence probability',
    'Chemotherapy': 'Chemotherapy - Receiving chemotherapy reduces recurrence risk',
    'Marital_Status': 'Marital Status - Married status associated with better prognosis',
    'Surgery': 'Surgery Type - Radical surgery reduces recurrence risk compared to conservative surgery'
}
...
if option == "🔍 Single Sample Prediction":
    ...
    st.markdown("Enter 6 key clinical factors to predict 5-year recurrence risk of cervical cancer.")

    # Create two-column layout
    col1, col2 = st.columns(2)

    with col1:
        st.subheader("Risk-Increasing Factors (Higher values = higher recurrence risk)")
        LODDS = st.slider(
    ...
    with col2:
        st.subheader("Risk-Reducing Factors (Higher values = lower recurrence risk)")
    ...
    if st.button("🚀 Start Prediction", type="primary", use_container_width=True):
        if model is None:
            ...
            with st.spinner("Analyzing..."):
                ...
                prediction_label = "High Recurrence Risk" if prediction == 1 else "Low Recurrence Risk"
                ...
                with col_result2:
                    st.metric("5-Year Recurrence Probability", f"{probability:.2%}")
                ...
                fig_gauge = go.Figure(go.Indicator(
                    ...
                    domain={'x': [0, 1], 'y': [0, 1]},
                    title={'text': "5-Year Recurrence Risk (%)"},
                    gauge={
                        'axis': {'range': [0, 100]},
                        'bar': {'color': "darkred"},
                        'steps': [
                ...
                try:
                    ...
                    if probability > 0.7:
                        ...
                        **High Risk (Recurrence Probability > 70%)**:
                        ...
                        st.warning("""
                        **Medium Risk (Recurrence Probability 30%-70%)**:
                        ...
                        st.success("""
                        **Low Risk (Recurrence Probability < 30%)**:
    ...
    with tab1:
        ...
        if 'feature_importance' in feature_info:
            ...
            **Feature Importance Explanation**:
            ...
            - Positive correlation means feature increases recurrence risk, negative means decreases risk
            """)
    ...
    with tab2:
        ...
        for feature in selected_features:
            feature_info_data.append({
                ...
                'Relationship to Recurrence': 'Positive' if feature in ['LODDS', 'FIGO_Stage', 'TumorSize_cm'] else 'Negative',
                'Typical Range': {
                    'LODDS': '0.0-8.0',
                    'FIGO_Stage': '1.0-4.0',
        ...
        1. **LODDS (Lymph Node Density and Size)**
           - **XGBoost Gain**: +0.458 (Most important recurrence predictor)
           - **Clinical Significance**: Measures lymph node involvement extent
           - **Recurrence Feature**: Higher LODDS indicates more extensive lymph node metastasis
           - **Reference Values**: 
             - Low risk: < 2.0
             - High risk: > 4.0
        ...
        2. **FIGO_Stage (International Federation of Gynecology and Obstetrics Stage)**
           ...
           - **Recurrence Feature**: Higher stage indicates more advanced disease
           - **Staging Explanation**: 
        ...
        3. **TumorSize_cm (Tumor Size in Centimeters)**
           ...
           - **Recurrence Feature**: Larger tumors have higher metastatic potential
           - **Clinical Threshold**: Tumors > 4cm have significantly higher recurrence risk
...
# ====================== 5. User Guide Page ======================
    ...
    st.markdown("""
    ## 📖 Cervical Cancer 5-Year Recurrence Prediction System User Manual

    ### 1. System Overview
    This system is based on the XGBoost machine learning model, trained on cervical cancer clinical datasets.
    It predicts 5-year recurrence risk of cervical cancer using 6 key clinical factors and provides SHAP interpretability analysis.

    ### 2. Main Functions

    #### 🔍 Single Sample Prediction
    - **Function**: Predict 5-year recurrence risk for individual patients based on input features
    - **Steps**:
    ...
    - **Output**:
      - Prediction Result (Low/High Recurrence Risk)
    ...
    |---------|--------------|--------------|-----------------------|
    | `LODDS` | Positive | +0.458 | Lymph Node Density and Size, most important recurrence predictor |
    | `FIGO_Stage` | Positive | +0.313 | Clinical stage, higher stage = higher recurrence risk |
    | `TumorSize_cm` | Positive | +0.227 | Tumor size, larger tumors increase recurrence probability |
    | `Chemotherapy` | Negative | -0.183 | Receiving chemotherapy reduces recurrence risk |
    | `Marital_Status` | Negative | -0.127 | Married status associated with better prognosis |
    | `Surgery` | Negative | -0.113 | Radical surgery reduces recurrence risk |

    ### 4. Result Interpretation Guide

    #### Risk Level Classification:
    - **Low Risk (<30%)**: Low probability of recurrence, recommend routine follow-up
    - **Medium Risk (30%-70%)**: Need further evaluation, recommend specialist consultation
    - **High Risk (>70%)**: High probability of recurrence, recommend immediate medical attention

    #### SHAP Value Interpretation:
    - **Positive SHAP Value**: Increases recurrence risk
    - **Negative SHAP Value**: Decreases recurrence risk
...
st.sidebar.markdown("---")
st.sidebar.info("""
**Cervical Cancer 5-Year Recurrence Prediction System**  
